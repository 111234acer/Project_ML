...
public class PlayerController_Copy : MonoBehaviour
{
    ...
    public PlayerHealth_Copy health;
    public PlayerAttack attack;
    [HideInInspector] public bool isDashing = false;            // 루미아 캐릭터 대시 중인지 확인

    PhotonView pv;

    public Vector3 lastMoveWorldDir { get; private set; }

    private void Awake()
    {
        pv = GetComponent<PhotonView>();
        if (health == null) health = GetComponent<PlayerHealth_Copy>();
    }
    void Start()
    {
